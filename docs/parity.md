...
| Revision inspection | Partial | Metadata, changed-file stat/names, bounded full patch, `Ctrl-B` terminal-native blame for the selected tracked status file, and `Ctrl-G` bounded decorated ASCII graph across all refs/branches/tags; graph rows are selectable with `↑`/`↓` or `j`/`k`, and `Enter` opens the exact selected commit | Persistent revision-local command set, parent navigation, branch/ref actions from graph rows, and deeper interactive blame controls |
| Whole-file stage/unstage | Partial | `s`/`u`; `S` stages all tracked changes excluding untracked; `U` clears index changes while preserving the worktree; unborn branches, renames/deletions, and literal paths are handled | Multi-file selection and Magit's context-sensitive defaults |
...
| Diff/log | Partial | `d`/`l` provide bounded status/revision/range diffs, unified terminal comparisons, graph/decorated history, current/other/all/branch/tag matching logs, reflogs, shortlogs, and refs | Persistent revision buffers, richer filters, and Magit's full graph/revision UI |
...
| Submodules/worktrees | Partial | `o` manages add/init/update/sync/deinit/remove/list/fetch submodules; `Z` manages reviewed add/new-branch/move/remove/prune plus list/lock/unlock worktrees; subtree and sparse-checkout transients provide typed terminal-native workflows | Submodule status presentation, recursive multi-module selection, and every Git worktree/subtree/sparse option |
...
The next core parity milestones are persistent history/revision navigation with branch/ref actions, multi-file selection, and deeper terminal-native blame controls. These provide more daily terminal value than adding Emacs-only integrations out of order.
