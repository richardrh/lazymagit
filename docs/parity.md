...
| Revision inspection | Partial | Metadata, changed-file stat/names, bounded full patch, `Ctrl-B` terminal-native blame, and selectable bounded commit logs/ASCII graphs; `↑`/`↓` or `j`/`k` selects commits, `Enter` inspects, existing history transients act on the exact selected revision, `p` opens its first parent, and `Esc` restores graph selection | Merge-parent selection, branch/ref actions from graph rows, and deeper interactive blame controls |
