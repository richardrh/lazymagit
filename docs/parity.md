...
| Commit | Partial | `c` transient supports create, extend, amend, reword, fixup, squash, alter, augment, and revise; target-based variants default instantly to the selected status, graph, revision, or blame commit and use a reviewed stale-safe HEAD/index/worktree contract; typed author/date/signoff/reuse/reedit/signing options; bounded terminal-native multiline message editing; hooks and explicit signing consent | `--verbose` preview, external editor workflows, and all Magit message-editing edge cases |
...
| Stash | Partial | `z` transient workflows, including non-destructive `z Z` tracked index/worktree snapshots, plus folded status section, `j z` jump, stable OID rows, and bounded lazy patch detail | Index-only/worktree-only snapshots and stashes, WIP refs, and exact section presentation edge cases |
...
| Cherry-pick/revert | Partial | `A`/`a` and `V`/`v`: one or more newline-separated commits are resolved and reviewed in order with mainline/strategy/signoff, non-editor apply, and reviewed continue/skip/abort | Visual region/mark collection and editor-backed message editing |
