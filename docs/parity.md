...
| Revision inspection | Partial | Metadata, changed-file stat/names, bounded full patch, `Ctrl-B` terminal-native blame for the selected tracked status file, and `Ctrl-G` bounded decorated ASCII graph across all refs/branches/tags; graph rows are selectable with `↑`/`↓` or `j`/`k`, `Enter` opens the exact selected commit, `p` opens its first parent, and `Esc` restores the preserved graph selection | Persistent revision-local command set, merge-parent selection, branch/ref actions from graph rows, and deeper interactive blame controls |
