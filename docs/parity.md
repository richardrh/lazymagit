...
| Revision inspection | Partial | Metadata, changed-file stat/names, bounded full patch, `Ctrl-B` terminal-native blame for the selected tracked status file, and `Ctrl-G` bounded decorated ASCII graph across all refs/branches/tags; both use terminal-safe detail rendering | Selectable graph commits, persistent revision-local commands, parent navigation, and deeper interactive blame controls |
...
The next core parity milestones are selectable persistent history/revision navigation, multi-file selection, and deeper terminal-native blame controls. These provide more daily terminal value than adding Emacs-only integrations out of order.
