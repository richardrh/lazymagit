...
Git behavior and transient suffixes use this Magit baseline. Runtime navigation
and top-level invocation keys follow Doom Magit/Evil instead of stock Emacs
bindings; the README lists those keys. The stock manifest is provenance only.
...
| Revision inspection | Partial | Metadata, changed-file stat/names, bounded full patch, selectable `Alt-b` blame lines, and selectable bounded commit logs/ASCII graphs; navigation selects exact commits, `Enter` inspects them, history transients use the selected revision, `Alt-p` opens its first parent, and `Esc` restores the prior graph or blame cursor | Merge-parent selection, branch/ref actions from graph rows, and parent/earlier-revision blame traversal |
...
| Commit | Partial | `c` transient supports create, extend, amend, reword, fixup, squash, alter, augment, and revise; target-based variants use a reviewed stale-safe HEAD/index/worktree contract; typed author/date/signoff/reuse/reedit/signing options; Vim-style message buffer with persistent drafts, paste, visual edits, undo/redo, and a staged diff; amend/reword preload HEAD; hooks and explicit signing consent | `--verbose` infix, external editor workflows, and full Vim/Magit message-editing parity |
| GitHub PR messages | Adapted | `Alt-r` loads or creates the current branch's PR; shared Vim-style title/body buffer, local template defaults, base and draft/ready controls, persistent drafts, explicit publication review, and result URL through authenticated `gh`; no automatic push/fork | Full Forge, GitLab, Markdown preview, review/comments/checks, and multiple-PR selection |
...
| Stash | Partial | `Z` transient workflows, including non-destructive `Z Z` tracked index/worktree snapshots, plus folded status section, stable OID rows, and bounded lazy patch detail | Index-only/worktree-only snapshots and stashes, WIP refs, and exact section presentation edge cases |
...
| Cherry-pick/revert | Partial | `A`/`a` and `_`/`-`: one or more newline-separated commits are resolved and reviewed in order with mainline/strategy/signoff, non-editor apply, and reviewed continue/skip/abort | Visual region/mark collection and editor-backed message editing |
| Reset | Partial | `o` quick mixed reset and `O` transient: reviewed mixed/soft/hard/keep/index/worktree/file reset with stale-state rejection | Magit's context-sensitive defaults and revision browser |
...
| Submodules/worktrees | Partial | `'` manages add/init/update/sync/deinit/remove/list/fetch submodules; `*` manages reviewed add/new-branch/move/remove/prune plus list/lock/unlock worktrees; subtree and sparse-checkout transients provide typed terminal-native workflows, including reviewed cone/non-cone setup, sparse index, set/add/reapply, and explicit disable restoration | Submodule status presentation, recursive multi-module selection, and every Git worktree/subtree/sparse option |
| Process output | Partial | Backtick bottom process pane, bounded sanitized chronological command/output transcript, failure auto-open, OSC52 copy request | Magit's full process lifecycle controls and persistent process buffer behavior |
